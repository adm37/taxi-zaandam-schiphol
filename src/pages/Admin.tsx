      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <button
              ...
              className="px-4 py-2 rounded-xl border border-stone-300 bg-white text-stone-700 hover:bg-stone-50 text-sm sm:text-base"
            >
            ...
            <button
              ...
              className="px-4 py-2 rounded-xl bg-stone-900 text-white hover:bg-stone-800 text-sm sm:text-base"
            >
              Uitloggen
            </button>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
          <div className="md:hidden p-4 space-y-4">
            {isLoading ? (
              <div className="rounded-xl border border-stone-200 bg-stone-50 px-4 py-6 text-center text-stone-500">
                Laden...
              </div>
              ...
              sortedBookings.map((booking) => {
                ...
                return (
                  <article key={booking.id} className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <p className="text-xs text-stone-500">Binnengekomen</p>
                        <p className="text-sm font-semibold text-stone-900 mt-1">{new Date(booking.createdAt).toLocaleString('nl-NL')}</p>
                      </div>
                      <button
                        type="button"
                        onClick={() => handleDeleteBooking(booking.id)}
                        disabled={deletingBookingId === booking.id}
                        className="w-9 h-9 rounded-lg border border-red-300 text-red-700 hover:bg-red-50 disabled:opacity-50 inline-flex items-center justify-center"
                    ...
                    <div className="mt-4 grid grid-cols-1 gap-3 text-sm">
                      <div>
                        <p className="text-xs text-stone-500">Klant</p>
                        <p className="font-semibold text-stone-900">{booking.name}</p>
                        <p className="text-stone-700">{booking.phone}</p>
                      </div>

                      <div>
                        <p className="text-xs text-stone-500">Rit</p>
                        <p className="font-medium text-stone-900">{booking.pickup}</p>
                        <p className="text-stone-700 mt-1">{booking.destination}</p>
                        <p className="text-stone-600 mt-1">{booking.date} om {booking.time}</p>
                        ...
                        {nightRideLabel && <p className="text-stone-500 mt-1">{nightRideLabel}</p>}
                      </div>

                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="rounded-lg bg-stone-50 px-3 py-2">
                          <p className="text-stone-500">Rittype</p>
                          <p className="text-stone-800 font-medium mt-1">{getTripTypeLabel(booking.paymentMethod)}</p>
                        </div>
                        <div className="rounded-lg bg-stone-50 px-3 py-2">
                          <p className="text-stone-500">Prijs</p>
                          <p className="text-stone-800 font-semibold mt-1">EUR {formatMoney(booking.totalPrice)}</p>
                        </div>
                        <div className="rounded-lg bg-stone-50 px-3 py-2">
                          <p className="text-stone-500">Passagiers</p>
                          <p className="text-stone-800 font-medium mt-1">{booking.passengers}</p>
                        </div>
                        <div className="rounded-lg bg-stone-50 px-3 py-2">
                          <p className="text-stone-500">Koffers</p>
                          <p className="text-stone-800 font-medium mt-1">{booking.suitcases}</p>
                        </div>
                      </div>

                      <div>
                        <p className="text-xs text-stone-500">Voertuig en betaling</p>
                        <p className="text-stone-700">{booking.vehicleType}</p>
                        <p className="text-stone-700 mt-1">{booking.paymentMethod}</p>
                      </div>
                    </div>
                  </article>
                );
                ...
                {isLoading ? (
                  ...
                  sortedBookings.map((booking) => {
                    ...
                    <tr key={booking.id} className="border-t border-stone-100 text-sm text-stone-700 align-top">
                      ...
                      <td className="px-3 py-3 font-semibold text-stone-900">EUR {formatMoney(booking.totalPrice)}</td>
